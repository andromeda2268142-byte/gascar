...
const nav: Array<{ id: Section; label: string; icon: string }> = [
  ...
  { id: 'finance', label: 'Finance', icon: '(value?: string | null) {
  if (!value) return '—';
...
export default function AdminScreen() {
  ...
  const [newServiceCategory, setNewServiceCategory] = useState<'repair' | 'towing'>('repair');
  ...
  async function createService() {
    if (!newServiceName.trim()) {
      Alert.alert('Service name required', 'Enter a service name.');
      return;
    }

    setWorkingId('new-service');
    try {
      const { error } = await getSupabaseClient().rpc('gascars_admin_create_service', {
        p_name: newServiceName.trim(),
        p_category: newServiceCategory,
        p_group_name: newServiceGroup.trim() || null,
        p_description: null,
        p_keywords: [],
      });
      if (error) throw error;
      ...
      Alert.alert('Could not create service', error instanceof Error ? error.message : 'Please try again.');
    } finally {
      setWorkingId(null);
    }
  }

  async function grantCredits(businessId: string, credits = 10) {
    await runAction(
      'credit-' + businessId,
      () => getSupabaseClient().rpc('gascars_admin_grant_credits', {
        p_business_id: businessId,
        p_credits: credits,
        p_reason: 'Admin portal grant',
      }),
    );
  }
  ...
  const overview = (
    <View>
      ...
      <View style={styles.opsGrid}>
        ...
        <Pressable onPress={() => setSection('finance')} style={styles.opsCard}>
          <Text style={styles.opsIcon}>$</Text>
          <Text style={styles.opsTitle}>Credits & finance</Text>
          <Text style={styles.opsText}>Review provider credit balances and issue controlled admin grants.</Text>
        </Pressable>
        <Pressable onPress={() => setSection('audit')} style={styles.opsCard}>
          <Text style={styles.opsIcon}>≡</Text>
          <Text style={styles.opsTitle}>Audit log</Text>
          <Text style={styles.opsText}>Review privileged administrative changes made on the platform.</Text>
        </Pressable>
      </View>
  ...
  const servicesView = (
    <View style={styles.listGap}>
      <View style={styles.createCard}>
        <Text style={styles.createTitle}>Add service to catalog</Text>
        <View style={desktop ? styles.createRowDesktop : undefined}>
          <TextInput
            value={newServiceName}
            onChangeText={setNewServiceName}
            placeholder="Service name"
            placeholderTextColor="#999B94"
            style={[styles.createInput, desktop && { flex: 1 }]}
          />
          <TextInput
            value={newServiceGroup}
            onChangeText={setNewServiceGroup}
            placeholder="Group (Brakes, Electrical...)"
            placeholderTextColor="#999B94"
            style={[styles.createInput, desktop && { flex: 1 }]}
          />
        </View>
        <View style={styles.actions}>
          <SmallButton label="Repair" active={newServiceCategory === 'repair'} onPress={() => setNewServiceCategory('repair')} />
          <SmallButton label="Towing" active={newServiceCategory === 'towing'} onPress={() => setNewServiceCategory('towing')} />
          <SmallButton label="Add service" active disabled={workingId === 'new-service'} onPress={() => void createService()} />
        </View>
      </View>
  ...
  const financeView = (
    <View style={styles.listGap}>
      {businesses.length === 0 ? (
        <View style={styles.empty}><Text style={styles.emptyTitle}>No provider wallets yet</Text><Text style={styles.emptyText}>Wallets appear when businesses begin using credits.</Text></View>
          ...
          <View key={business.id} style={styles.rowCard}>
            ...
            <View style={styles.actions}>
              <SmallButton
                ...
                onPress={() => void grantCredits(business.id, 10)}
              />
            </View>
          </View>
        );
  ...
  const auditView = (
    <View style={styles.listGap}>
      {auditEntries.length === 0 ? (
        <View style={styles.empty}><Text style={styles.emptyTitle}>No admin activity yet</Text><Text style={styles.emptyText}>Privileged actions will be recorded here.</Text></View>
      ) : auditEntries.map((entry) => (
        <View key={entry.id} style={styles.rowCard}>
          <Text style={styles.rowTitle}>{entry.action}</Text>
          <Text style={styles.rowMeta}>{formatDate(entry.created_at)} · Actor {entry.actor_id.slice(0, 8)}</Text>
...
const styles = StyleSheet.create({
  ...
  createCard: { backgroundColor: colors.white, borderWidth: 1, borderColor: colors.line, borderRadius: 18, padding: 14 },
  createTitle: { color: colors.ink, fontSize: 12.5, fontWeight: '950', marginBottom: 10 },
  createRowDesktop: { flexDirection: 'row', gap: 8 },
  createInput: { minHeight: 46, borderRadius: 13, borderWidth: 1, borderColor: colors.line, backgroundColor: '#F8F6F0', paddingHorizontal: 12, color: colors.ink, fontSize: 11.5, marginBottom: 8 },
  balancePill: { minWidth: 72, borderRadius: 14, backgroundColor: colors.limeSoft, paddingHorizontal: 10, paddingVertical: 7, alignItems: 'center' },
  balanceValue: { color: colors.ink, fontSize: 15, fontWeight: '950' },
  balanceLabel: { color: colors.muted, fontSize: 7.5, marginTop: 1 },
